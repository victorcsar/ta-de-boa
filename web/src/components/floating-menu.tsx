...
export default function FloatingMenu() {
    const menuItems = [
        { href: "/home", label: "Inicio", icon: <Grip width={26} height={26} stroke="currentColor" /> },
    ...
    return (
        <div className="fixed  w-full bottom-0 flex bg-custom-primary">
            {menuItems.map((item, index) => (
                <Link 
                 key={index} 
                 href={item.href} 
                 className={`${actualPath == item.href ? `bg-white text-custom-primary` : `bg-custom-primary text-white`}  flex flex-col items-center w-full py-2 gap-1`}
                >
                    {item.icon}
